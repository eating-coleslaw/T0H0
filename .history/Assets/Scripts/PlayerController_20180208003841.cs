...
public class PlayerController : MonoBehaviour {
	...
	bool IsStandingStill() {
		if ( (Mathf.Abs(velocity.x) < 0.01)&& (Mathf.Abs(velocity.y) < 0.01) && controller.collisions.below) {
			return true;
		} else {
			return false;
		}
	...
	bool IsRunning() {
		if ( controller.collisions.below && (Mathf.Abs(velocity.y) < 0.01) && (Mathf.Abs(velocity.x) > 0.01|| directionalInput.x != 0) ) {
			return true;
		} else {
			return false;
		}
